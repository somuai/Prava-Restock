...
def test_slack_outbox_migration_reaches_new_head(tmp_path, monkeypatch) -> None:
    ...
    assert "slack_deliveries" in inspect(database.engine).get_table_names()
    with database.engine.connect() as connection:
        assert connection.execute(text("SELECT version_num FROM alembic_version")).scalar_one() == "20260722_04"
